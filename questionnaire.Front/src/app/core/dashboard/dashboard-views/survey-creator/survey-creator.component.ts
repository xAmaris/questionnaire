...
import { SendSurveyDialog } from 'src/app/data/shared.data';
...
export class SurveyCreatorComponent
  ...
  openMoveControlDialog(question: FormGroup, arrayName: string): void {
    ...
    this.openMoveDialog(array, nameArr).subscribe(res => {
      if (res) {
        ((question.controls.FieldData as FormGroup)
          .controls[0] as FormGroup).controls[
          arrayName
        ] = this.setPositionOnMove(
          ((question.controls.FieldData as FormGroup).controls[0] as FormGroup)
            .controls[arrayName],
          res,
          nameArr.position
        );
  ...
  openMoveDialog(array: any, nameArr: any): Observable<any> {
  ...
  setPositionOnMove(array: any, res: any, controlName: any): FormArray {
    ...
    for (let i = 0; i < length; i++) {
      ...
      (clonedMoveList[i] as FormGroup)['controls'][controlName].setValue(i);
    }
    return clonedArr;
  }
  ...
  setNewQuestionPositions(index: number, object?: FormGroup) {
    ...
    if (index !== length - 1) {
      ...
      for (let i: number = newIndex; i < length; i++) {
        (questionList[i] as FormGroup).controls.QuestionPosition.setValue(
          i + 1
        );
      }
    }
  ...
  setCurrentQuestionPositions(
    ...
    for (let i: number = index; i < length; i++) {
      (array[i] as FormGroup)['controls'].QuestionPosition.setValue(i);
    }
  }
  ...
  choiceSwitch(select: any, fieldData: any) {
    switch (select) {
      case 'single-choice':
      case 'multiple-choice':
      case 'dropdown-menu':
  ...
  gridSwitch(select: any, fieldData: any) {
    switch (select) {
      case 'single-grid':
      case 'multiple-grid':
        break;
  ...
  showSurvey(): void {
    const string: string = '/app/survey/viewform/t/' + this.id;
