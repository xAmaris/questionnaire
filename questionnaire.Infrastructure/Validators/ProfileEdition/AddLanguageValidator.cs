...
namespace questionnaire.Infrastructure.Validators.ProfileEdition {
    public class AddLanguageValidator : AbstractValidator<AddLanguage> {
        public AddLanguageValidator () {
            RuleFor (reg => reg.Name)
                ...
                .WithMessage ("Name of language should contain only letters with one space between words");
            RuleFor (reg => reg.Proficiency)
                .NotNull ()
                .MinimumLength (3)
                .Matches (@"^[a-zA-ZàáąâäãåąčććęęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšśžÀÁÂÄÃÅĄČĆĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð'-]+(\s{1}[a-zA-ZàáąâäãåąčććęęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšśžÀÁÂÄÃÅĄČĆĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð'-]+)*$")
                .WithMessage ("Proficiency should contain only letters with one space between words");
        }
    }
}